#Este código acessa a API da plataforma de joos Steam e cria uma base de dados contendo cada jogo a uma taxa de
#aproximadamente 200 jogos a cada 5 minutos
...
def main():
    ...
    while len(apps_remaining_deque) > 0:
        ...
        try:
            ...
            appreviews_req = requests.get(f"https://store.steampowered.com/appreviews/{appid}?json=1")

            if appdetails_req.status_code == 200 and appreviews_req.status_code == 200:
                appdetails = appdetails_req.json()
                appdetails = appdetails[str(appid)]
                appreviews = appreviews_req.json()

            elif appdetails_req.status_code == 429 or appreviews_req.status_code == 429:
            ...
            elif appdetails_req.status_code == 403 or appreviews_req.status_code == 403:
        ...
        if 'query_summary' in appreviews:
            query_summary = appreviews['query_summary']
            appdetails_data['review_score'] = query_summary.get('review_score_desc', 'no_reviews')
            # Add additional review metrics
            appdetails_data['review_metrics'] = {
                'total_reviews': query_summary.get('total_reviews', 0),
                'positive_reviews': query_summary.get('total_positive', 0),
                'negative_reviews': query_summary.get('total_negative', 0),
                'score_value': query_summary.get('review_score', 0),
                'score_description': query_summary.get('review_score_desc', 'no_reviews')
            }
        else:
            appdetails_data['review_score'] = 'no_reviews'
            appdetails_data['review_metrics'] = None
